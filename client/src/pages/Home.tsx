  ...
  const content = (
    <div className="dashboard-shell">
      ...
      {active !== "overview" && <section className="content-stack detail-view" aria-labelledby="detail-heading"><div className="detail-toolbar"><div><p className="eyebrow">REMOTE OPERATIONS</p><h2 id="detail-heading">{navItems.find((item) => item.id === active)?.label}</h2></div><div className="search-box"><Search size={15} /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Filter records…" /></div></div>
        ...
        {active === "sessions" && selectedSessionId !== null && <Card className="panel-card session-history"><CardHeader className="panel-header"><div><p className="eyebrow">SESSION HISTORY</p><CardTitle>Evidence trail for session #{selectedSessionId}</CardTitle></div></CardHeader><CardContent>{historyQuery.isLoading && <p className="muted">Loading message history…</p>}{historyQuery.isError && <div className="dashboard-error" role="alert">Unable to load this session history.</div>}{!historyQuery.isLoading && !historyQuery.isError && (historyQuery.data?.length ?? 0) === 0 && <p className="muted">No messages recorded for this session.</p>}<div className="history-stack">{historyQuery.data?.map((message) => <div className={`history-message history-${message.role}`} key={message.id}><strong>{message.role}</strong><p>{message.content}</p><small>{message.status} · {new Date(message.createdAt).toLocaleString()}</small></div>)}</div></CardContent></Card>}
