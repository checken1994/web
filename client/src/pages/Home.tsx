...
export default function Home() {
  ...
  const setJobEnabled = async (id: number, enabled: boolean) => {
    setMutationError(null);
    try { await toggleJob.mutateAsync({ id, enabled }); await jobQuery.refetch(); } catch (error) { setMutationError(error instanceof Error ? error.message : "Scheduled job update failed."); }
  };

  const createJobAction = async () => {
    const name = jobName.trim();
    if (!name || createJob.isPending) return;
    setMutationError(null);
    try {
      await createJob.mutateAsync({ name, cron: jobCron.trim(), description: jobDescription.trim() || undefined });
      setJobName(""); setJobDescription(""); await jobQuery.refetch();
    } catch (error) { setMutationError(error instanceof Error ? error.message : "Scheduled job creation failed."); }
  };

  const registerArtifactAction = async () => {
    if (!artifactFile || registerArtifact.isPending) return;
    setMutationError(null);
    if (artifactFile.size > 8 * 1024 * 1024) { setMutationError("Artifact exceeds the 8 MiB limit."); return; }
    try {
      ...
      await registerArtifact.mutateAsync({ name: artifactFile.name, mimeType: artifactFile.type || "application/octet-stream", base64, sessionId: selectedSessionId ?? undefined, provenance: { source: "dashboard-upload", operation: "artifact.register" } });
      setArtifactFile(null); await artifactQuery.refetch();
  ...
  const content = (
    <div className="dashboard-shell">
      ...
      {active !== "overview" && <section className="content-stack detail-view" aria-labelledby="detail-heading"><div className="detail-toolbar"><div><p className="eyebrow">REMOTE OPERATIONS</p><h2 id="detail-heading">{navItems.find((item) => item.id === active)?.label}</h2></div><div className="search-box"><Search size={15} /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Filter records…" /></div></div>
        ...
        {active === "models" && <div className="cards-grid">{modelQuery.isLoading && <div className="empty-card"><div className="empty-icon"><Radio className="spin" size={20} /></div><h3>Loading models</h3><p>Reading the server-side model catalog…</p></div>}{modelQuery.isError && <div className="dashboard-error" role="alert">Unable to load the model catalog. No model selection is available.</div>}{!modelQuery.isLoading && !modelQuery.isError && serverModels.length === 0 && <div className="empty-card"><div className="empty-icon"><Sparkles size={20} /></div><h3>No models available</h3><p>The server catalog returned no selectable models.</p></div>}{!modelQuery.isLoading && !modelQuery.isError && serverModels.map((model) => <Card className="panel-card model-card" key={model.name}><CardContent><div className={`model-orb orb-${model.accent}`}><Sparkles size={20} /></div><h3>{model.name}</h3><p className="muted">{model.provider} · {model.state}</p><div className="model-stat"><span>Routing</span><strong>Server-side only</strong></div><Button className="primary-button w-full" onClick={() => chooseModel(model.name)}>Set preferred</Button></CardContent></Card>)}</div>}
        {active === "artifacts" && <Card className="panel-card"><CardHeader className="panel-header"><div><p className="eyebrow">EVIDENCE INPUT</p><CardTitle>Register an artifact</CardTitle></div><Badge className="badge-dark">server-side S3</Badge></CardHeader><CardContent><div className="form-grid"><label className="sr-only" htmlFor="artifact-file">Artifact file</label><Input id="artifact-file" type="file" onChange={(event) => setArtifactFile(event.currentTarget.files?.[0] ?? null)} /><Button onClick={registerArtifactAction} disabled={!artifactFile || registerArtifact.isPending}>{registerArtifact.isPending ? "Uploading…" : "Register artifact"}</Button></div><p className="muted">Maximum 8 MiB. Bytes are uploaded server-side and provenance is recorded.</p></CardContent></Card>}
        ...
        {active === "jobs" && <Card className="panel-card"><CardHeader className="panel-header"><div><p className="eyebrow">HEARTBEAT CONTROL</p><CardTitle>Create a scheduled job</CardTitle></div><Badge className="badge-dark">admin-gated</Badge></CardHeader><CardContent><div className="form-grid"><label className="sr-only" htmlFor="job-name">Job name</label><Input id="job-name" value={jobName} onChange={(event) => setJobName(event.target.value)} placeholder="Job name" /><label className="sr-only" htmlFor="job-cron">UTC cron expression</label><Input id="job-cron" value={jobCron} onChange={(event) => setJobCron(event.target.value)} placeholder="0 0 * * * *" /><label className="sr-only" htmlFor="job-description">Job description</label><Input id="job-description" value={jobDescription} onChange={(event) => setJobDescription(event.target.value)} placeholder="Description (optional)" /><Button onClick={createJobAction} disabled={!jobName.trim() || createJob.isPending}>{createJob.isPending ? "Creating…" : "Create job"}</Button></div><p className="muted">Six-field UTC cron. Creation is audited and the Heartbeat task is created server-side.</p></CardContent></Card>}
        {active === "jobs" && <Card className="panel-card"><CardContent className="p-0"><div className="list-stack">{jobQuery.isLoading && <div className="empty-card"><div className="empty-icon"><Radio className="spin" size={20} /></div><h3>Loading scheduled jobs</h3><p>Reading owner-scoped Heartbeat records…</p></div>}{jobQuery.isError && <div className="dashboard-error" role="alert">Unable to load scheduled jobs. No toggle action is available.</div>}{!jobQuery.isLoading && !jobQuery.isError && serverJobs.length === 0 && <div className="empty-card"><div className="empty-icon"><Clock3 size={20} /></div><h3>No scheduled jobs</h3><p>Heartbeat-backed schedules will be listed here after an admin creates one.</p></div>}{serverJobs.map((job) => <div className="list-row" key={job.id}><span className="row-icon"><Clock3 size={16} /></span><span className="row-main"><strong>{job.name}</strong><small>{job.schedule} · last run: {job.lastStatus}</small></span><span className="status-badge"><StatusDot tone={job.enabled ? "green" : "amber"} />{job.enabled ? "Enabled" : "Disabled"}</span><Button size="sm" variant="outline" onClick={() => setSelectedJobId(job.id)}>Runs</Button><Button size="sm" variant="outline" onClick={() => setJobEnabled(job.id, !Boolean(job.enabled))} disabled={toggleJob.isPending}>{job.enabled ? "Disable" : "Enable"}</Button></div>)}</div></CardContent></Card>}
        ...
        {active === "audit" && <Card className="panel-card"><CardContent className="p-0"><div className="list-stack">{auditQuery.isLoading && <div className="empty-card"><div className="empty-icon"><Radio className="spin" size={20} /></div><h3>Loading audit activity</h3><p>Reading immutable-style owner-scoped records…</p></div>}{auditQuery.isError && <div className="dashboard-error" role="alert">Unable to load audit activity.</div>}{!auditQuery.isLoading && !auditQuery.isError && serverAudits.length === 0 && <div className="empty-card"><div className="empty-icon"><ShieldCheck size={20} /></div><h3>No audit events yet</h3><p>Critical operations are recorded server-side with actor, action, status and timestamp.</p></div>}{serverAudits.map((event) => <div className="list-row" key={event.id}><span className="row-icon"><ShieldCheck size={16} /></span><span className="row-main"><strong>{event.action}</strong><small>{event.targetType}{event.targetId ? ` #${event.targetId}` : ""} · {new Date(event.createdAt).toLocaleString()}</small></span><span className="status-badge"><StatusDot tone={event.status === "completed" ? "green" : event.status === "failed" ? "red" : "amber"} />{event.status}</span></div>)}</div></CardContent></Card>}
      </section>}
    </div>
  );
