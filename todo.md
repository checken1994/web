...
- [ ] Complete automated accessibility coverage for all authenticated dashboard routes, including axe labels/contrast, keyboard focus order and prefers-reduced-motion behavior; connected-browser axe passes for all routes, but focus/reduced-motion automation and standalone storage-state execution remain pending.
...
- [ ] Run the committed authenticated axe/focus harness with a real external Playwright storage-state file; syntax and fail-closed missing-state behavior pass, but no state file is available in this sandbox.
...
- [x] Add a committed authenticated DOM accessibility harness for Overview, Sessions, Agents, Models, Artifacts, Jobs and Audit; it accepts external storage-state, checks axe/name/tabindex/reduced-motion metadata, and fails closed when state is absent. Actual state-backed execution remains pending.
- [ ] Run automated reduced-motion and real keyboard traversal checks per authenticated route; source contract and harness checks are committed, but runtime media emulation and a real storage-state execution remain pending.
