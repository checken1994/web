...
Accessibility/runtime fix evidence: session rows now render as keyboard-accessible `div[role=button]` with Enter/Space handling, so the child Cancel button is no longer nested inside a button element. Authenticated Sessions reload rendered all three completed records with the corrected semantics; no active Cancel action was available because no session remained queued/running.
